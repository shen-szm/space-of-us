...
const userCookieName = "mapofus_user";
...
const decodeUserPayload = (value: string): UserPayload | null => {
  try {
    const parsed = JSON.parse(Buffer.from(value, "base64url").toString("utf8")) as unknown;
    if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) return null;
    const payload = parsed as Partial<UserPayload>;
...
const createUserToken = (username: string) => {
  const payload = Buffer.from(
    JSON.stringify({
      username,
      exp: Date.now() + siteMaxAgeSeconds * 1000,
    }),
    "utf8",
  ).toString("base64url");
  const signature = sign(payload);
...
const verifyUserToken = (token?: string) => {
  ...
  const payload = decodeUserPayload(payloadValue);
  if (!payload || payload.exp < Date.now()) return null;
...
export const getSessionUsername = (request: NextRequest) =>
  hasSiteSession(request) ? verifyUserToken(request.cookies.get(userCookieName)?.value) : null;
...
export const setAuthCookies = (response: NextResponse, role: AuthRole, username?: string) => {
  ...
  if (username) {
    ...
    if (userToken) {
      response.cookies.set(userCookieName, userToken, {
        httpOnly: true,
        maxAge: siteMaxAgeSeconds,
        path: "/",
        sameSite: "lax",
...
export const clearAuthCookies = (response: NextResponse, role: AuthRole | "all" = "all") => {
  if (role === "site" || role === "all") {
    response.cookies.delete(siteCookieName);
    response.cookies.delete(userCookieName);
  }
